...
# ---------------------------------------------------------------------
# CMYKW-aware PAT helpers (optional use)
# ---------------------------------------------------------------------
PAT_TOKEN_RE = re.compile(r"__PAT_([cmykw]+)__", re.IGNORECASE)

def strip_pat(name: str) -> str:
    """Remove __PAT_<token>__ (CMYKW) from a label, preserving the rest."""
    return PAT_TOKEN_RE.sub("", name or "").strip()

def extract_pat_token(name: str) -> Optional[str]:
    """Return the CMYKW token if present, else None."""
