...
Previous palette behavior plus experimental-viewer-style whole-model Move/Rotate
interaction in the existing embedded build-plate viewer.
"""
...
_SPEC = importlib.util.spec_from_file_location("layerloom_gui_v62_runtime_for_v64", _V62_PATH)
...
class V64InteractiveQtInteractor(_V58.QtInteractor):
...
class EmbeddedPVViewerV64(_BASE_VIEWER):
    ...
    def set_callbacks(
        ...
        if on_move_preview is not None:
            self.on_move_preview = on_move_preview
    ...
    def _v64_has_active_drag(self) -> bool:
    ...
    def _ensure_experimental_interaction_observers(self):
        if not self.plotter or self._v64_observer_ids:
            return
        try:
            vtk = _V58._get_vtk()
    ...
    def _apply_move_preview(self, delta_xy) -> None:
        ...
        try:
            if self.selection_outline_actor is not None:
                self.selection_outline_actor.SetUserMatrix(self._vtk_matrix(mat))
    ...
    def _update_move_drag(self, display_pos, *, qt_coords: bool = True) -> bool:
        ...
        if callable(self.on_move_preview):
            try:
                self.on_move_preview(delta_xy)
            except Exception:
                pass
        return True
    ...
    def _update_rotation_drag(self, display_pos) -> bool:
        ...
        if callable(self.on_rotation_preview):
            self.on_rotation_preview(rotation, drag["axis"], angle)
        else:
            self._apply_temp_transform(rotation)
        self._show_rotation_feedback(drag["axis"], angle, pos)
        if callable(self.on_rotation_status):
            self.on_rotation_status(f"Rotate {drag['axis'].upper()} {angle:.1f} deg")
        return True

    def _finish_rotation_drag(self) -> bool:
        if self._active_rotation is None:
            return False
        rotation = _V58.orthonormalize_rotation(self._active_rotation.get("rotation", _V58.np.eye(3)))
    ...
    def _on_mouse_move(self, *_args):
        if self._v64_has_active_drag() and not self._left_button_is_down():
    ...
    def _qt_mouse_press(self, event, *, use_interactor_pos: bool = False):
        ...
        if self.tool_mode == "move":
            ...
            name, _actor, _cell_id, _pick_pos = self._pick_model_at_vtk_display_exact(vtk_display)
            if name is None:
                self._active_move = None
    ...
    def _rebuild_selection_outline(self):
        ...
        try:
            ...
            self.selection_outline_actor = self.plotter.add_mesh(
                outline,
                name="__v64_selection_outline__",
                color="#f2c94c",
                line_width=4,
                lighting=False,
                pickable=False,
...
_V58.EmbeddedPVViewer = EmbeddedPVViewerV64
...
class QtAssignColorsApp(_V62.QtAssignColorsApp):
    def __init__(self):
        ...
        self._preview_bounds_corner_cache = None
        self._rotation_preview_base_matrix = None
        super().__init__()
        self.setWindowTitle("LayerLoom — Color Assigner v64")
    ...
    def _apply_actor_transform(self, rotation):
        ...
        bounds = self._preview_bounds_after_actor_matrix(mat)
        if bounds is not None:
            mat[2, 3] -= float(bounds[4])
            dx, dy = self._actor_matrix_xy_clamp_delta(bounds)
            mat[0, 3] += dx
            mat[1, 3] += dy
        try:
        ...
        for actor in self.viewer.actors_by_name.values():
            try:
                ...
                pass
        try:
            if hasattr(self.viewer, "_render_drag_preview"):
                self.viewer._render_drag_preview()
            else:
    ...
    def _preview_min_z_after_actor_matrix(self, matrix):
        bounds = self._preview_bounds_after_actor_matrix(matrix)
        return None if bounds is None else float(bounds[4])

    def _preview_bounds_after_actor_matrix(self, matrix):
        mins = []
        maxs = []
        mat = _V58.np.asarray(matrix, dtype=_V58.np.float64)
        corners_by_actor = self._preview_actor_bounds_corners()
        for corners in corners_by_actor:
            try:
                transformed = (mat @ corners.T).T[:, :3]
                mins.append(_V58.np.min(transformed, axis=0))
                maxs.append(_V58.np.max(transformed, axis=0))
            except Exception:
                pass
        if not mins:
            return None
        mn = _V58.np.min(_V58.np.vstack(mins), axis=0)
        mx = _V58.np.max(_V58.np.vstack(maxs), axis=0)
        return float(mn[0]), float(mx[0]), float(mn[1]), float(mx[1]), float(mn[2]), float(mx[2])

    def _preview_actor_bounds_corners(self):
        if self.viewer is None:
            return []
        if self._preview_bounds_corner_cache is not None:
            return self._preview_bounds_corner_cache
        actors = getattr(self.viewer, "actors_by_name", {})
        corners = []
        for name, actor in actors.items():
            try:
                ...
                if dataset is None:
                    continue
                b = tuple(float(v) for v in dataset.bounds)
            except Exception:
                continue
            corners.append(
                _V58.np.asarray(
        ...
        self._preview_bounds_corner_cache = corners
        return corners

    def _actor_matrix_xy_clamp_delta(self, bounds):
        margin = self._plate_margin_mm()
        plate_w = float(_V58.PLATE_WIDTH_MM)
        plate_d = float(_V58.PLATE_DEPTH_MM)
        xmin, xmax, ymin, ymax = [float(v) for v in bounds[:4]]
    ...
    def _sync_viewer_tool_mode(self):
        if self.viewer is not None:
            ...
            self.viewer.set_callbacks(
                ...
                on_move_preview=self._preview_move_delta,
    ...
    def _preview_move_delta(self, delta_xy):
        # Keep status/readout responsive during actor-matrix move preview without
        # rebuilding the file-backed preview on every mouse event.
        try:
            delta = _V58.np.asarray(delta_xy, dtype=_V58.np.float64).reshape(2)
            self.status_bar.showMessage(f"Move X {delta[0]:.1f} mm, Y {delta[1]:.1f} mm")
        except Exception:
            pass
...
def main():
    ...
    _V58.APP_TITLE = "LayerLoom — Color Assigner v64"
    ...
    if not _V58._QT_OK:
        raise RuntimeError("Qt dependencies are unavailable; cannot launch v64.")
