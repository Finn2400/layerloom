...
Default pipeline:
  ...
  2. Slice each part into repeating CMYKW Z-bands (`slice_repeating`)
  3. Tag each band with __PAT_<c|m|y|k|w>__
...
def assemble_by_color(
    ...
    # --------------------------------------------------------------
    # Step 3: slice each part according to its token run (CMYKW-aware)
    # --------------------------------------------------------------
    sliced_parts = []
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        ...
        description="Slice and group baked 3MF parts by CMYKW color bands."
    )
