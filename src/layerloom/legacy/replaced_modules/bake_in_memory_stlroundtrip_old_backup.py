...
# -*- coding: utf-8 -*-
"""
bake_in_memory_stlroundtrip.py (v8-prusaunitfix)
------------------------------------------------
...
def gather_leaves_mm(obj_map, obj_key, tf_mm, parent_file, file_scale_mm):
    ...
    for c in comps.findall("m:component",NS):
        ...
        # ⬇ No scaling for Prusa/Bambu — leave local translations untouched
        local_mm=local[:]
...
def read_prusa_sidecar(zf):
    names=[n for n in zf.namelist() if "Slic3r_PE_model.config" in n or "PrusaSlicer_model.config" in n or "OrcaSlicer_model.config" in n]
    if not names: return []
...
def pack_3mf(parts,out,title="baked_roundtrip"):
    ...
    for i,(lbl,verts,tris) in enumerate(parts,1):
        ...
        for x,y,z in verts:
            v=ET.SubElement(vs,M("vertex")); v.set("x",f"{x:.9f}".rstrip("0").rstrip(".")); v.set("y",f"{y:.9f}".rstrip("0").rstrip(".")); v.set("z",f"{z:.9f}".rstrip("0").rstrip("."))
        ts=ET.SubElement(m,M("triangles"))
        for a,b,c in tris:
            t=ET.SubElement(ts,M("triangle")); t.set("v1",str(a)); t.set("v2",str(b)); t.set("v3",str(c))
    ...
    with zipfile.ZipFile(out,"w",compression=zipfile.ZIP_DEFLATED) as z:
        ...
        z.writestr("_rels/.rels",b'<?xml version="1.0"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rel0" Type="http://schemas.microsoft.com/3dmanufacturing/2013/01/3dmodel" Target="3D/3dmodel.model"/></Relationships>')
...
def bake_roundtrip(input_path,output_path,keep_stls=False,debug=False,safe_volumes=False,growth_limit=2.0):
    ...
    for item in build.findall("m:item",NS):
        ...
        for leaf,tf_final,src in gather_leaves_mm(obj_map,base_key,tf_mm,main,scales):
            ...
            label=leaf.get("name") or f"id_{oid}"
