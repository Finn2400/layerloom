...
# CMYKW-aware PAT run (allows multi-letter runs; we will tag per-band with a single char)
PAT_RE = re.compile(r"__PAT_([cmykw]+)__", re.IGNORECASE)
ALPHABET = set("cmykw")  # sanitize any unexpected characters
...
def weave_pipeline(
    ...
    # 5. Slice by CMYKW token sequence
    ...
    for i, (lbl, mesh) in enumerate(parts, 1):
        m = PAT_RE.search(lbl)
        # default to CMY cycle if no PAT present (keeps legacy behavior)
        run = (m.group(1).lower() if m else "cmy")
        # sanitize to allowed alphabet (drop any unknown characters)
