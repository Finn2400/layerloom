...
This folder contains the scripts, exact processed inputs, registration records,
and outputs used to measure photographed LayerLoom gamut prints.

The manuscript's aggregate expanded-gamut results are under
`expanded_gamut/`. They were produced by
`analyze_expanded_gamut_photos.py` from 46 registered photographs. The release
includes each exact post-crop/downsample `*_analysis_input.png`, 1,955
per-region measurement records, all registration coordinates and manual
overrides, and the aggregate tables and figures. Run
`../analysis/recompute_expanded_gamut_summary.py` from the archive root to
rebuild and verify the final summary tables.

The files in `inputs/`, `registration/`, and `results/` are auxiliary
single-image and mosaic analyses. They are retained as inspectable examples,
but they are not the source of the manuscript's final aggregate counts or hull
areas.
