...
Layer-fraction recipe & The normalized endpoint counts implied by a token; \texttt{cmy}, \texttt{myc}, and \texttt{ccmmyy} all encode the same 1:1:1 C:M:Y fraction. \\
...
The set of available woven colors is defined by a constrained palette built from a chosen endpoint-filament set and a small number of pattern parameters: maximum token length \(H\), maximum same-color run length, optional restrictions on how many distinct filaments may appear in a token, and the selected endpoint set. LayerLoom supports CMY workflows and optional black, white, neutral gray, light gray, orange, violet, and green endpoints. The photographic analysis includes both light-gray and darker neutral-gray endpoints.

A reported color recipe is defined by the reduced layer-fraction vector of a valid stack token, not by every possible ordering of that token. Thus, tokens such as \texttt{cmy}, \texttt{myc}, and \texttt{ccmmyy} are treated as the same expected recipe because they reduce to the same C:M:Y layer fraction of 1:1:1. Recipe counts reported here therefore describe unique layer-fraction recipes after collapsing redundant orderings and reducible repeats, not all possible stack-string permutations and not experimentally resolved printed colors.
...
Geometrically, LayerLoom constructs woven output by slicing selected parts or colored regions into successive Z-bands and reassigning those bands according to the chosen weave signature. Bands assigned to the same filament are accumulated into new output geometry, so the final woven model consists of per-filament objects containing noncontiguous but coordinated component layers. The color schedule therefore becomes part of the object itself: geometry describes both shape and the layer-level ordering by which differently colored filaments appear throughout the print. Compared with voxel-based or highly fragmented composition methods, this preserves longer continuous structures and remains compatible with ordinary mesh-based workflows \citep{vidimce2013openfab,willmott2023three}.
...
GLB inputs are imported into the same representation using their component geometry and color information. Component colors can then be mapped automatically to the nearest available LayerLoom weave recipe or assigned manually through the graphical interface. Each editable part is assigned a weave signature, or stack token. If a part name or metadata contains a stack-token tag, such as \texttt{\_\_PAT\_cmymy\_\_}, that token is used directly; otherwise, the GUI may assign a token manually or by matching preserved source color metadata to the LayerLoom palette.
...
Arrangements A4 and A5 add a new physical filament color, so they test the cost of increasing filament-channel count rather than only displayed-color count.

For each model arrangement (Fig.~\ref{fig:performance}; Table~S3), models were sliced using a 0.12~mm layer height and a 0.24~mm initial layer height.
...
Since PrusaSlicer did not include a default 0.12~mm profile for the Prusa XL, we modified the 0.10~mm DETAIL profile to use a 0.12~mm layer height and 0.24~mm first-layer height.
The reported metrics are total print time in hours and total filament used in grams.
These measurements and estimates test a specific practical consequence of geometry-encoded layer scheduling: slicers plan toolpaths by physical filament assignment on each layer, not by the nominal number of displayed colors in the model.
...
The CMY tests demonstrate the basic layer-weaving principle, but the practical palette also depends on the available endpoints. LayerLoom's token representation is not limited to CMY: any loaded filament can be used as an endpoint in the same layer-fraction grammar. We therefore tested secondary and neutral/value filaments by replacing one CMY endpoint at a time with white, light gray, dark neutral gray, black, orange, violet, or green (Fig.~\ref{fig:ovg-neutral-mosaic}; Figs.~S1--S3; Table~S5). These prints sample three-filament slices of a larger endpoint space rather than the full theoretical combined palette. In the broader theoretical full-palette rule set \((H \leq 6,\ \mathrm{run} \leq 5,\ \leq 5\ \mathrm{colors})\), CMYKW yields 386 unique expected recipes, CMYKWN yields 812, and the one-neutral CMYKWN+OVG palette yields 4,647 theoretical recipes (Table~\ref{tab:palette-family-counts}). These counts describe expected layer-fraction recipes, not experimentally resolved printed colors.
...
Secondary and neutral/value endpoints allow some saturated, dark, light, or low-saturation colors to be represented with nearby physical filaments rather than long, high-contrast CMY-only sequences.
This can reduce striation and viewing-angle sensitivity. The expanded endpoint samples therefore show that the token representation scales beyond CMY and provide the printed source data for the calibrated color-coverage analysis.
...
LayerLoom shows that color expansion in multi-material FFF can be achieved by encoding layer structure directly into printable geometry. Its central contribution is a portable 3MF representation in which the model carries both the object shape and the layer sequence needed to fabricate each woven color. In the tested workflows, many woven recipes remained distinguishable after fabrication, showing that the expanded recipe space produced practical printed colors rather than only theoretical layer-fraction combinations.
...
Because LayerLoom changes the deposited color sequence rather than the underlying polymer, woven and single-color coupons printed from the same PLA would be expected to have similar mechanical properties unless the alternating color interfaces weakened the part. Consistent with this expectation, we found no meaningful difference in ultimate tensile strength between woven and single-color coupons in either tested orientation (Supplementary Figs.~S9--S11 and Tables~S6--S7).

Overall, LayerLoom shows that expanded-color FFF does not require expanded hardware or direct G-code manipulation. By encoding layer-level material schedules directly into 3MF geometry, the model carries the color program needed to generate woven printed colors, and tested compatible slicer-printer workflows execute that program as ordinary multi-material geometry. The results support geometry-level color scheduling as a portable workflow where Z registration and filament mapping are preserved; they do not establish universal compatibility or target-color accuracy for every printer, filament, or slicer profile.



\section*{Data and Code Availability}
An anonymized versioned repository accompanying this revision is available at \url{ANONYMIZED_REPOSITORY_URL}. It contains the exact revised manuscript and supplement source, the LayerLoom source snapshot, palette and configuration files, representative 3MF inputs and outputs, benchmark data, calibrated image-analysis inputs and scripts, all 20 raw Mark-10 workbooks, and the tensile-analysis script used for this revision.
